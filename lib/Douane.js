...
function validateFunction(req) {
    return function(done) {
        ...
        async.parallel(fns, function(err, results) {
            ...
            return done(req.douane.resultFormatter(errors));
        });
    };
};
 ...
 * @param {Function} setter Updates the value of a parameter.
 */
function Validation(req, param, getter, setter) {
    this.req = req;
...
/**
* Defines a new validation method.
*
    ...
    Validation.prototype[methodName] = function() {
        ...
        this.contexts.forEach(function(context) {
            this._checks.push(function(done) {
                ...
                // Stop validation when an error occurred
                if (func.apply(func, [context].concat(args)) === false) {
                    ...
                    return done(null, context.req.douane.errorFormatter(context, failMsg, args));
                }
                return done();
            });
        }.bind(this));
