...
function checkFunction(req, getter) {
    return function(param) {
        var validation = new Validation(req, getter, param);
 ...
 *
 * Checks are executed in sequential order and validation stops at the first check that fails.
 *
 * @param {Object} req The request object.
 * @param {Function} getter A function returning the startpoint of the object.
 * @param {String} param The parameter locater string.
 */
function Validation(req, getter, param) {
    this.req = req;
    var setter = function(context, newValue) {
        setParam(getter, context.param, newValue);
    };
    this._checks = [];
    ...
    function generateParams(param) {
        ...
        var result = [];
        var match = /^(.+?)\[\]\.?(.*)$/.exec(param); // Find the first [], splitting param into two string
        var array = getParam(getter, match[1]);
        if (array) {
...
function getParam(getter, param) {
...
function setParam(getter, param, newValue) {
    var params = param.split('.').filter(function(elm) {
        return elm !== '';
    });
    ...
    if (params.length > 1) {
        ...
        var elm = getParam(getter, findParam);
...
Validation.prototype.setSanitizer = function(methodName, func) {
    Validation.prototype[methodName] = function() {
        ...
        this.contexts.forEach(function(context) {
            this._checks.push(function(done) {
                var newValue = func.apply(func, [context].concat(args));
                if (newValue != context.value) {
                    context.setter(context, newValue);
                }
                return done();
            });
        }.bind(this));
