...
Douane.setValidator('optional', null, function(ctx) {
    if (_.isNull(ctx.value) || _.isUndefined(ctx.value)) {
        ctx.finished = true;
    }
...
Douane.setValidator('maxElements', 'Must contain no more than {0} elements', function(ctx, maxElements) {
    return Array.isArray(ctx.value) && ctx.value.length <= maxElements; // TODO add test
});

Douane.setValidator('isBoolean', 'Must be a boolean', function(ctx) {
    return _.isBoolean(ctx.value);
});

Douane.setValidator('isNumber', 'Must be a number', function(ctx) {
    return _.isNumber(ctx.value);
});
...
Douane.setValidator('isArray', 'Must be an array', function(ctx, max) {
    return _.isArray(ctx.value);
});

Douane.setValidator('isObject', 'Must be an object', function(ctx, max) {
    return _.isObject(ctx.value);
});
